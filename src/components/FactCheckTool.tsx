...
function FactCheckTool() {
  ...
  const buildVerificationFlow = useCallback(
    (data: any, inputContent: string): VerificationStep[] => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ADDED: Validation for image input
      if (inputType === "image") {
        if (!imageFile) {
...
if (imageFile) {
  ...
  formData.append('input', input.trim());
      ...
      // ADDED: Use extracted text from image if available
      const inputContent =
        inputType === "image"
  ...
  // ADDED: Updated function to handle image type
  const handleInputTypeChange = (newType: "text" | "url" | "image") => {
    ...
    setTranslations({});
    setSelectedLanguage("en");
    // ADDED: Clear image states when switching
    setImageFile(null);
    setImagePreview("");
  };
      ...
      <div className="relative z-10 text-left max-w-5xl mx-auto w-full">
        ...
        <div className="bg-black/40 backdrop-blur-md border border-white/5 rounded-2xl p-6 max-w-5xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* ADDED: Conditional rendering for image input */}
            {inputType === "image" ? (
            ...
            <button
              type="submit"
              // ADDED: Updated disabled condition to handle image input
              disabled={
                ...
                >
                  ...
                  >
                    ...
                    <MiniMap
                      ...
                      nodeColor={(node) => {
                        const status = result.verificationFlow?.find(
                          (s) => s.id === node.id
                        )?.status;
